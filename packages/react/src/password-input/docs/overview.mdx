...
- use the autocomplete attribute for personal fields
...
- use maxlength to restrict password length - clearly indicate the format requirements within the label or hint text and show an error message instead.
...
<PasswordInput label="Password input: on [%_PALETTE_%] background" required />
```
