...
For accessibility reasons, the `DatePicker` component will always append the date format to the label.

When using the `DatePicker` component in scenarios where vertical space is not an issue, such as in forms for capturing user data, you can use the `hideDateFormatLabel` prop to hide the date format from the label.

When using the `hideDateFormatLabel`, you _must_ provide the date format `"(dd/mm/yyyy)"` via the hint text.
...
For accessibility reasons, the `DateRangePicker` component will always append the date format to the label.

When using the `DateRangePicker` component in scenarios where vertical space is not an issue, such as in forms for capturing user data, you can use the `hideDateFormatLabel` prop to hide the date format from the label.
