...
For accessibility reasons, the `DatePicker` component will always append the date format to the label. However, this prevents the label automatically appending `"(optional)"` to the label like other form components in AgDS.

When using the `DatePicker` component in a form for capturing user data, you can use the `hideLabelDateFormat` prop to hide the date format and automatically append `"(optional)"` like other form components in AgDS.

When using the `hideLabelDateFormat`, you _must_ provide the date format `"(dd/mm/yyyy)"` via the hint text.
