...
To fix this issue, you can use the `onInputChange` prop to keep track of the user’s input. The `value` prop can also be set to a string, which represents the value of the text input.
...
() => {
	...
	const onInputChange = (e) => {
		console.log(`onInputChange, e`, onInputChange, e);
		setValue(e);
	};
	const onChange = (e) => {
		console.log(`onChange, e`, onChange, e);
		setValue(e);
	};
