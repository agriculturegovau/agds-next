...
By default, `PageAlert` renders `PageAlertTitle` as an `h3`. If you need to change the title’s semantic tag to an `h2`, for instance, you can use the `PageAlertTitle` component in the `title` prop to explicitly set it. For information on maintaining proper heading levels refer to [Structuring headings and labels](/content/content-structure#structuring-headings-and-labels).
