...
- use to fetch results from a server – use [SearchBox](/components/search-box) or [Autocomplete](/components/autocomplete)
- hide the search icon or clear button
- use for global site search - use [SearchBox](/components/search-box).
...
<div aria-label="Site" role="search">
	<SearchInput label="Search input: on [%_PALETTE_%] background" />
</div>
```
