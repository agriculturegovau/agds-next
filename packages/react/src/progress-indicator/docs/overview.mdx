...
By default, the item with status `'doing'` will be marked as active. If you would like a different item to be marked as active, then apply an `isActive: true` property to the item.
Currently, only the `'doing'`, `'started'`, and `'blocked'` statuses may recieve the `isActive: true` property.
