...
If one of the steps in a multi-step form has a secondary task, use a separate page for the secondary task. Let users know where they are by using a level 2 step in the progress indicator. A level 2 step sits under the main step. There can only be a single level 2 step for each main step.
...
<ProgressIndicator
	...
	items={[
		{ href: '#', label: 'Introduction', status: 'done' },
		{
			...
			label: 'Organisations',
			status: 'started',
			levelTwoItem: { label: 'Add an organisation', href: '#' },
		},
		{ href: '#', label: 'Review and submit', status: 'blocked' },
	]}
/>
```
