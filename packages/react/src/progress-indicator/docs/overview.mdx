...
To indicate the currently active item, whether level 1 or level 2, pass in a matching path to the `activePath` prop. E.g. `activePath="/organistions/change-name"`.
...
If one of the steps in a multi-step form has a secondary task, such as adding an employee with name and email to a list of employees, use a separate page for the secondary task. Let users know where they are by using a level 2 step in the progress indicator. A level 2 step sits under the main step. There can only be a single level 2 step for each main step.
...
Let’s say that a user is completing a multi-step application form. They want to change their pre-filled details, so they select the link to change their details.

_IMAGE ONE GOES HERE_

The link takes the user to a separate page where they can change their details. This page is still within the main step, so a level 2 step is displayed in the progress indicator. This helps the user understand where they are in the process.

_IMAGE TWO GOES HERE_
