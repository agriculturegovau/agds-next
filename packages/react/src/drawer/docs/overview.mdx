...
The Drawer overlay defaults to 80% opacity however by adding `mutedOverlay={true}` the opacity can be reduced to 30%. This makes it easier for sighted users to see the content underneath the Drawer overlay so that context can more easily be retained with the main page content.

This is available for when sighted users with cognative disabilites need to retain context with the main page content while interacting with the Drawer content.

Only apply this when additional context with the main page content is required.
