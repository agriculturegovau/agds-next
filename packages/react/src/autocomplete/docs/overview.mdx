...
Use the `loadOptions` to load options from a remote source as the user types. This is a function that returns a promise, which is the set of options to be used once the promise resolves.
