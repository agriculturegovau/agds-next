...
- use for small lists – use [Radio](/components/radio) instead
- use for long lists – consider using [Combobox](/components/combobox) to allow the user to filter results.
...
<Select
	label="Select: on [%_PALETTE_%] background"
	options={[
		{ value: 'a', label: 'Option A' },
		{ value: 'b', label: 'Option B' },
		{ value: 'c', label: 'Option C' },
