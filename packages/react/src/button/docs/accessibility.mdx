...
Include `aria-label` or `aria-labelledby` when visible text alone is insufficient – for example, ‘Edit’ fits into a table, but ‘Edit record 2’ doesn’t. 
...
All buttons have accessible names (via the button text, aria-label, or aria-labelledby). 
