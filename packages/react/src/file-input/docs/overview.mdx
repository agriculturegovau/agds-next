...
- use this option when there are other form fields on the page
- when a drag and drop zone isn’t required
- if only one file is being uploaded

<DontHeading />

- if the user might need to remove the file after uploading
- if you don’t want the file to be uploaded immediately
- use for uploading multiple files
- if the user might benefit from a drag and drop zone
- if it’s the only form element on the page
