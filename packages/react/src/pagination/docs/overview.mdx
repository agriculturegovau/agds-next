...
The `Pagination` component accepts a `itemRangeText` prop, which is used to display the range of items currently being displayed. You can use the `generatePaginationRangeText` function to generate a string which follows the format of `1 - 10 of 458 items`.
...
- `onChangeItemsPerPage` - a function that is called when the number of items per page is changed
...
<Pagination
	...
	totalPages={10}
	generateHref={(page) => `/example?page=${page}`}
	itemRangeText="1-25 of 458 results"
	itemsPerPage={25}
	itemsPerPageOptions={[10, 25, 50, 100]}
	onChangeItemsPerPage={(itemsPerPage) => console.log(itemsPerPage)}
/>
```
