...
The `required` prop can be used to indicate that user input is required on the field before a form can be submitted. `required` fields do not show the ‘(optional)’
