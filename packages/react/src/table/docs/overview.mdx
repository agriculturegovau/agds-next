...
The `<TableRow>` component accepts a callback function that will run when the table row or cell is clicked. Please read the [Table accessibility document](/components/table/accessibility#tablerow-and-onclick) for more details.

To prevent the table row callback from executing unintentionally, the clicked target is checked and will only call the function if the target is a `<th>`, `<tr>` or `<td>`.

```jsx live
() => {
	const [selectedRows, setSelectedRows] = React.useState([]);
	const [targetedElement, setTargetedElement] = React.useState(null);
	const [toggleButtonState, setToggleButtonState] = React.useState({});

	function toggleRowSelection(rowIndex) {
		setTargetedElement('checkbox');
		if (selectedRows.includes(rowIndex)) {
			setSelectedRows((r) => r.filter((s) => s !== rowIndex));
		} else {
			setSelectedRows((r) => [...r, rowIndex]);
	...
	function handleClickToggleButton(index) {
		setTargetedElement('toggleButton');
	...
	return (
		<Stack gap={2}>
			<Box>
				...
				<Text as="p">
					{targetedElement === 'row' && 'onClick event called for <TableRow />'}
					{targetedElement === 'toggleButton' &&
						'onClick event called for <ToggleButton />'}
					{targetedElement === 'checkbox' &&
						'onClick event called for <Checkbox />'}
					{!targetedElement &&
						'No element detected, click on the table row to identify the clicked elements region.'}
				</Text>
			</Box>
					...
					<Table>
						...
						<TableBody>
							{Array.from(new Array(3).keys()).map((index) => {
								...
								return (
									<TableRow
										onClick={() => setTargetedElement('row')}
										...
										<TableCell>
											<Flex gap={1}>
												<ToggleButton
													...
													iconType="flag"
													label="Flag message"
													onClick={() => handleClickToggleButton(index)}
