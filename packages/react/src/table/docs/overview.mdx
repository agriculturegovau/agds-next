...
The `<TableRow>` component accepts a callback function that will run when the table row or cell is clicked. Refer to the [Table accessibility document](/components/table/accessibility#tablerow-and-onclick) for further information.

To prevent the table row callback from executing unintentionally, the clicked target is checked and will only run the callback if the target is a `<th>`, `<tr>` or `<td>`.
	...
	return (
		<Stack gap={2}>
			<Box>
				...
				<Text as="p">
					{targetedElement
						? `onClick event called for <${targetedElement} />`
						...
						<TableBody>
							{Array.from(new Array(3).keys()).map((index) => {
								...
								return (
									<TableRow
										key={index}
										onClick={() => setTargetedElement('TableRow')}
