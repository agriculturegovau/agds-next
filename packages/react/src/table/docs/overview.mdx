...
Background colours should only be used in scenarios where the paradigm is clearly understood and has many existing examples (e.g. email applications often show an email’s current state with colour). If the meaning is important, it should also be indicated in another way for non-sighted users.
...
<TableWrapper>
	<Table striped>
		<TableCaption>Support background row colours</TableCaption>
		<TableHead>
			<TableRow>
