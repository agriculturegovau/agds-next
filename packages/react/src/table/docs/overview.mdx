...
Frozen columns remain in place as the table is horizontally scrolled. You can freeze the first column, the last, or both. On small screens, all columns will automatically revert to a non-fixed state to prevent users from encountering hidden content.

_Note: see Storybook example for responsive behaviour._

```jsx live
<TableWrapper>
	<Table frozenColumns={[1, 5]} tableLayout="fixed">
		<TableHead>
			<TableRow>
				<TableHeader scope="col" width="20rem">
					Business name
