...
The Time picker component allows users to select a single Time via a calendar or text input. 

 

Learn more about how to meet WCAG success criteria when implementing time pickers. This makes them accessible and inclusive for users of all abilities and situations. 
...
Labels clearly identify the purpose of the time picker. 

The purpose of the time picker is understood and conveyed by assistive technologies. 
...
Input errors are detected and communicated to the user – for example, invalid time formats and out-of-range times. 
...
Clear label identifying the time picker. 
