...
Time picker is superset of [Combobox](/components/combobox), with additional functionality specific to entering times. It accepts all of Combobox’s props, with the exception of `options`.
...
By default, Time picker displays options from 12 am to 11:45 pm at 15 minute intervals. This can be customised by setting the `min`, `max` and `step` props. `min` and `max` accept at 24 hour time string in the `HH:mm` format (such as `'09:00'`). `step` accepts a number of minutes between `1` and `60` to create the options at appropriate intervals between the `min` and `max` times.
