...
Time input is a superset of [Text input](/components/text-input), with additional functionality specific to entering times. It accepts all of Text input’s props, with changes to `onChange` and `value`.
...
() => {
	...
	return (
		<TimeInput
			invalid={invalid}
			label="Time input: on [%_PALETTE_%] background"
			message="Enter a valid time"
			onChange={onChange}
			value={value}
		/>
