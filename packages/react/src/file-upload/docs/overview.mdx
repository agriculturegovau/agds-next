...
Unlike the `value` prop, which expects an array of `File` objects, `existingFiles` expects an array of objects with a name, size and optional thumbnail url and other properties.
