...
In the event you need to focus the alert, set the `tabIndex` to `-1`. This will allow programmatic focusing and will render a focus ring to aid accessibility.
...
() => {
	...
	return (
		<Stack gap={2}>
			<Box alignSelf="start">
				<Button onClick={() => sectionAlertRef.current.focus()}>
					Focus the alert
				</Button>
			</Box>
