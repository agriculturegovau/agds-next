...
Each menu item can have an optional icon on the left to help users scan the menu faster. Menu items can also have a [Notification badge](/components/badge#notificationbadge) on the right to indicate the number of notifications.

The app layout sidebar should be hidden by default while users are completing multi-page forms to reduce distractions (as seen in the following example).
