...
The header component can be used with no log, one logo, or two logos when the [Australian Government Brand Guidelines](https://www.pmc.gov.au/publications/australian-government-branding-guildelines) for Program or International branding are met. Each logo can be a link when an `href` and `secondHref` are defined.
...
<Box palette="dark">
	<Header
		heading="Co-brand header"
		subline="Co-brand subline"
		logo={<Logo />}
		background="bodyAlt"
		size="sm"
		href="#first-href"
