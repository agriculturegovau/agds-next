...
Header supports one, two, or no logos. Each logo can be a link when an `href` and `secondHref` are defined.

_Note: The first logo's `href` will be assigned to the heading text as well._

```jsx live
<Box palette="dark">
	<Header
		heading="Co-branded example"
		subline="Partners on and off the field"
		logo={<Logo />}
		background="bodyAlt"
		size="sm"
		href="#first-href"
