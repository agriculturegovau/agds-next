...
() => {
	const generateOptions = (number) => {
		const result = [];
		for (let i = 0; i < number; i++) {
			const countryIndex = i % NAME_OPTIONS.length;
			const fullName = `${NAME_OPTIONS[countryIndex].fullName} ${
				Math.floor(i / NAME_OPTIONS.length) + 1
			}`;
			const jobTitle = `${NAME_OPTIONS[countryIndex].jobTitle} ${
				Math.floor(i / NAME_OPTIONS.length) + 1
			}`;
			const status = `${NAME_OPTIONS[countryIndex].status} ${
				Math.floor(i / NAME_OPTIONS.length) + 1
			}`;
			result.push({
				label: fullName,
				...
				jobTitle,
				status,
				unreadMessageCount: NAME_OPTIONS[countryIndex].unreadMessageCount,
			});
		}
		return result;
	};
