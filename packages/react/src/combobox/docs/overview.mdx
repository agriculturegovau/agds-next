...
While we don’t recommend extremely large datasets, the Combobox components should comfortably handle many options and cater for most use-cases. However, you can test the performance of your dataset size using the example below.

```jsx live
() => {
	const [numOfOptions, setNumOfOptions] = React.useState(
		COUNTRY_OPTIONS.length
	);
	const [options, setOptions] = React.useState(COUNTRY_OPTIONS);
	const [value, setValue] = React.useState(null);
	...
	return (
		<FormStack>
			...
			/>
			<Button onClick={() => setOptions(generateOptions(numOfOptions))}>
				Set
			</Button>
			<Combobox
				label="Select country"
