...
Make sure you include a [meaningful text label](/packages/button/rationale#meaningful-text-label) with or without an icon on a button.

**Do's**

- Use a button if the user needs to perform an action.
- [Align buttons left](/packages/button/rationale#align-buttons-left) from the most to least important action.
...
- [Don't use disabled buttons](/packages/button/rationale#dont-use-disabled-buttons).
...
An icon can be used next to a [meaningful text label](/packages/button/rationale#meaningful-text-label) in a button to help users quickly recognise what the button does.
...
`ButtonLink` adopts the Link component from your chosen router framework, which you can set in the [Core component](/packages/core).
