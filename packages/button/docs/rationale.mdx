...
A [meaningful button label](/packages/forms/button/content) with a noun and verb makes the action that happens next obvious to all users and easy to scan.

## Don’t use disabled buttons

A [disabled button](/packages/forms/button#disabled) can confuse the user as:
...
Always use a visible button label with an icon unless there’s insufficient space.

Buttons with icons only can confuse users who may mistake the icon for an incorrect action. For example, an ‘x’ can close a page but also indicate an error to the user.
