...
description: The yourGov application is designed to showcase the components and patters of the design system. It enables usability testing with a diverse range of users.
---
...
The patterns included in yourGov demonstrate important details of component use and integration that are difficult to show is standard design system documentation. Details demonstrated in the app like layout spacing, focus management, keyboard navigation, form validation and correction are some of the concepts that impact usability and accessibility.

### Entering a service

The entry point for yourGov uses a unauthenticated page with a [Hero banner](/components/hero-banner) with Sign in call to action button.
...
It’s important to move focus to the title of the current step as the user navigates through the form. We do this so users can start interacting straight away.

#### Single-page form

This demonstration of the [Single-page form](/templates/single-page-form) pattern multi-question form. It illustrates how an invitation can be sent to add someone as an employee within an application.
...
- Conditional reveal of relation question/s
- Address input
- Grouped fields
...
<h4 id="form-getting-started-page">Form “Getting started” page</h4>
...
- what the user needs to complete the form (e.g. documents or account details)
...
yourGov includes a complex data table that displays staff records. The table supports advanced interactions such as sorting, [filtering](/patterns/search-filters), individual and batch actions. Users are able to see, compare, and manage a dataset.

#### Filtering data

yourGov includes the use of our [Search filters pattern](/patterns/search-filters) to demonstrate how this pattern works with data. The filters allow users to refine a dataset based on multiple criteria. This showcases how the pattern behaves, both in terms of usability and accessibility.

#### Table batch actions

yourGov includes a list of staff who are requesting access to the system. Users have the choice of applying actions on each row or multiple rows via batch actions.
...
- Details component and [Drawer](/components/drawer) for additional info​
- Complex tables actions – filtering, batch processes​
- Overall simplicity​
...
In our first round of testing, we had the [conditional reveal](/patterns/conditional-reveal) question right under the radio option the user had just picked – thinking that asking a related question right after their choice would be logical.​

In 2022, usability testing showed a positive response to conditionally revealed content between radio buttons in a group. However, in 2024 testing, some users found the pattern confusing. One participant completed a conditionally revealed input, then continued to select a different radio option. This caused them to lose the conditionally entered information. This resulted in confusion and disorientation.

Some users found it confusing, saying that they thought the field would be below the list of all radio options. One user assumed that the radio buttons below was a new question.​

We reverted to moving below the radio group and observed no issues.​
...
Feedback indicated that users expected to return directly to the Review and submit step after making edits. Some users attempted to navigate back using the progress indicator, but this approach did not save their changes. In response, a Review and submit sub-step pattern was introduced so users can edit sections of the form without leaving the Review and submit sub-page.
...
2. Apply the [Warn before leaving pattern](/pattern/warn-before-leaving) to the progress indicator. This prevent accidental data loss when navigating away from a page.

#### Switching entities with the sidebar

At the end of the first task, participants were asked to switch to another entity linked to their account.​
...
​
In later rounds, we updated the labels and replaced the entity dashboard ‘home’ icon with a entity-related icon to improve understanding.​
