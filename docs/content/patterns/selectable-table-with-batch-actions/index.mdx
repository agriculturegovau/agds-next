...
Only [Buttons](/components/button) should be placed inside of the `TableBatchActionsBar` component. Do not place form components inside the `TableBatchActionsBar` component, as they can introduce inconsistencies as well as complexities related to validation and submission.

Action buttons should trigger [Drawers](/components/drawer) when pressed, which should contain the form related for each action button.

Each action button should be applicable for every row in the Table. If certain actions can not be performed on every selectable row, do not include this action in the `TableBatchActionsBar`.
