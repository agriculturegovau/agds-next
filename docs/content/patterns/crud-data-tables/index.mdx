...
Use a repeatable file upload pattern when uploading an undefined list of files of different categories or when specific metadata for each file is required. Metadata can be associated with uploaded files when using this pattern to support back stage processing.
