...
- summarise form validation errors as a list inside of a [Page alert component](/components/page-alert) with the 'error' tone
- place the [Page alert component](/components/page-alert) at the top of the page and focus immediately after a submission attempt
- validate a form when the user attempts to submit
- omit the error summary if there is only one form validation error - focus the invalid field instead.
...
- display an error summary if only one validation error
- depend only on client-side validation - also use server-side validation

## When to validate

A form should validate when a user attempts to submit the form. This should occur when the user presses the submit button at the bottom of the form.
...
It is important that users do not lose the data they have entered when they attempt to submit the form. Retaining the userss input allows them to correct any errors without starting over.

## Summarising form validation errors

Form validation errors should be summarised as a list inside of a [Page alert component](/components/page-alert) with the 'error' tone. The [Page alert component](/components/page-alert) should be placed at the top of the form and focused immediately after a submission attempt to ensure it is visible and announced to screen reader users.
...
If there is only one form validation error, the error summary should not be displayed and the invalid form field should be focused immediately after a submission attempt.

## Invalid form fields

Form validation errors should also be displayed as invalid field error messages. Use the same error message in the invalid field and in the error summary so they are consistent and reduce the cognitive effort needed to understand what has happened.
