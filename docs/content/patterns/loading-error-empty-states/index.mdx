...
version: 1.0.0
storybookPath: /story/patterns-data-loading--cards
---
...
This state could potentially be caused by the user visiting the page for the first time or by applying filters that do not match any results.

This should not be confused with the [Error state](#error-state).

If this empty state has been caused by, use a [Button](/components-button) component to provide the user with a way of clearing any settings.
