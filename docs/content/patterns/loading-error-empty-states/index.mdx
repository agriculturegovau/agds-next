...
- a user visting a service for the time and there being no data to display
- a user applying settings or filters which ends up in no results being found

Depending on what caused the empty state, provide clear messaging and a helpful action to get rid of the empty state via a [Button](/components/button) component.
