...
description: Adds two new, replacement Date picker components, 18 new icons, Improves the accessibility and documentation for many components, increases the performance of Box and Combobox, a new Feature link list component and eight new icons, updated release of the yourgov app, plus various bug fixes and improvements.
---
...
- `AppLayoutSidebarNav`: Update to cater for new native mouse events on links and buttons.
...
- Remove callout in control group story as we no longer support this pattern.
...
- Add `aria-expanded` attribute to button. Make the example date more relevant. Add superseded announcement to docs.
- Fix Escape keydown listener to not close parent `Drawer`.
- Fix all `act`/state warnings in tests.
...
- Add `aria-expanded` attribute to button. Make the example date more relevant. Fix 2nd calendar dropdown updating to the wrong calendar. Add
  superseded announcement to docs.
- Fix Escape keydown listener to not close parent `Drawer`.
- Fix all `act`/state warnings in tests.
...
- Add `event` to argument to `onClick` function prop.
- Fix all `act`/state warnings in tests.

### [File input](/components/file-input)

- Fix button click not firing when ref is passed. Focus button when hidden input is focused.
...
- Add `maxWidth='container|containerLg'` prop.
- Update `MainNavList` to cater for new native mouse events on links and buttons.
...
- Add `onMouseEnter`, `onMouseLeave`, `onMouseOut` and `onMouseOver` props. Update `onClick` to match `BaseButton`’s signature.
